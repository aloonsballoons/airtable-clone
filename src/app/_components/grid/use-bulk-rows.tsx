...
import { MAX_ROWS, BULK_ROWS } from "~/lib/constants";
...
interface UseBulkRowsOptions {
  ...
  getRowsQueryKey: (tableId: string) => RouterInputs["row"]["getRows"];
  onBulkSuccess?: () => void;
}
...
export function useBulkRows({
  ...
  const addRows = api.row.addRows.useMutation({
    ...
    onSuccess: (data, variables, context) => {
      ...
      if (data?.newTotalCount != null && !isSingleRow) {
        utils.row.getRows.setInfiniteData(queryKey, (old) => {
          if (!old) return old;
      ...
      void utils.row.getRows.invalidate(queryKey, { refetchType: 'none' });
      void utils.table.getTableMeta.refetch({ tableId });
    },
    onError: (error, variables, context) => {
      ...
      void utils.row.getRows.invalidate(context.queryKey);
    },
  });
