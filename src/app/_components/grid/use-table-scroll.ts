...
import {
  ...
  ROW_SCROLLING_RESET_DELAY_MS,
  ROW_PREFETCH_AHEAD,
  SPARSE_PREFETCH_BUFFER,
  SPARSE_SCROLL_THRESHOLD,
} from "~/lib/constants";
...
export function useTableScroll({
  ...
  const handleScroll = useCallback(
    (el: HTMLDivElement) => {
      ...
      // Account for header that's sticky at the top — visible rows start after ROW_HEIGHT
      const scrollTop = el.scrollTop;
      const clientHeight = el.clientHeight;
      const scrollTopAdjusted = Math.max(0, scrollTop - ROW_HEIGHT);
      const firstVisibleRow = Math.floor(scrollTopAdjusted / ROW_HEIGHT);
      const lastVisibleRow = Math.ceil(
        (scrollTopAdjusted + clientHeight) / ROW_HEIGHT,
      );
      const bufferSize = SPARSE_PREFETCH_BUFFER;
      ...
      if (
        Math.abs(prefetchStart - last.start) >= SPARSE_SCROLL_THRESHOLD ||
        Math.abs(prefetchEnd - last.end) >= SPARSE_SCROLL_THRESHOLD
      ) {
