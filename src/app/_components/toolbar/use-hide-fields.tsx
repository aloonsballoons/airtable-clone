...
import type { ColumnFieldType, IconSpec } from "~/lib/types";
import { getHideFieldsIconSpec } from "~/lib/column-icons";
...
// Icon helpers
// ---------------------------------------------------------------------------
// Icon specs are imported from ~/lib/column-icons
...
export function useHideFields({
  ...
  const hideFieldsLayout: HideFieldsLayout = useMemo(() => {
    const rows = hideFieldColumns.map((column, index) => {
      ...
      const iconLeft = HIDE_FIELDS_TEXT_LEFT - (iconSpec.gap ?? 0) - iconSpec.width;
